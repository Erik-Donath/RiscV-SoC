...
"""
this file is importet from this Project https://github.com/quartiq/Phaser_STFT_Pulsegen/blob/master/fft_generator_migen.py
"""
...
class FFTCore(Module, csr.AutoCSR):
    """FFT mit korrekten LiteX CSRs"""
    
    def __init__(self, n=32, width_i=16, width_o=16, width_int=16,
                 width_wram=16, ifft=False, input_bitreversed=False):
        
        # FFT-Core instanziieren
        self.submodules.fft = Fft(
            n=n, ifft=ifft, width_i=width_i, width_o=width_o,
            width_int=width_int, width_wram=width_wram,
            input_bitreversed=input_bitreversed
        )
        
        log2n = self.fft.log2n
        
        # ========== CSRs (KORREKTE IMPORTS!) ==========
        self.control  = CSRStorage(2, fields=[
            CSRField("start", size=1, description="Start FFT"),
            CSRField("ifft", size=1, description="IFFT mode")
        ])
        
        self.scaling  = CSRStorage(log2n, description="Scaling mask")
        self.status   = CSRStatus(2, fields=[
            CSRField("busy", size=1),
            CSRField("done", size=1)
        ])
        
        self.x_in_adr  = CSRStorage(log2n, description="Input address")
        self.x_in_data = CSRStorage(width_i*2, description="Input data")
        self.x_in_we   = CSRStorage(1, description="Input write enable")
        
        self.x_out_adr  = CSRStorage(log2n, description="Output address")
        self.x_out_data = CSRStatus(width_o*2, description="Output data")
        
        # ========== VERBINDUNGEN ==========
        self.comb += [
            # Parameter
            self.fft.scaling.eq(self.scaling.storage),
            
            # Input/Output
            self.fft.x_in.eq(self.x_in_data.storage),
            self.fft.x_in_adr.eq(self.x_in_adr.storage),
            self.fft.x_in_we.eq(self.x_in_we.storage),
            self.fft.x_out_adr.eq(self.x_out_adr.storage),
            self.x_out_data.status.eq(self.fft.x_out),
            
            # Status
            self.status.fields.busy.eq(self.fft.busy),
            self.status.fields.done.eq(self.fft.done)
        ]
        
        # Start-Puls-Logik (fallende Flanke)
        start_q = Signal()
        self.sync += [
            start_q.eq(self.control.fields.start),
            self.fft.start.eq(~start_q & self.control.fields.start)
        ]