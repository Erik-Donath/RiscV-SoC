...
wir wollten unserer Projekt uhrspr\"unglich so aufbauen das wir alles auf opensource toolchains aufbauen auch mit dem hintergrund das man sich um die Gowin-EDA zu bekommen bei Gowin selber registrieren muss und dabei seine informationen preisgeben muss was wir einfach nicht wollten und auch fuer die sp\"ateren nutzer nicht wollten also wollten wir eine opensource toolchain verwenden bestehend aus yosys(ein routing tool), nextpnr-himbaechel(zum generieren eines bitstreams anhand der routing file und der .cst file) so wie zum schluss apycula zum \"ubersetzen des bitstreams in ein f\"ur unser board angepassten bitstream.
\\
Soweit der Theoretische plan dieser hat aber in der Realit\"at nicht funktioniert aus dem einfachen grund, das diese toolchain absoulut nicht optimiert ist sprich f\"ur einfache Projekte funktioniert diese toolchain unfassbar gut aber f\"ur komplexere und gr\"ossere Projekte funktioniert dieseToolchain einfach nicht da sie die LUTS einfach nicht gut optimiert.
\\
Also musste der Plan B genutzt werden der voraussieht, das wir die GOWIN-EDA nutzen da aber keiner der Entwickler seine Informationen Hergeben wollte haben wir nach m\"oglichkeiten gesucht die GOWIN-EDA ohne Informationen Preiszugeben herunterladen zu k\"onnen. Hier hat Erik beim Reverse Engeenieren der Website gesehen das man um die executable der GOWIN-EDA Herunterladen zu k\"onnen keine Seasion braucht und somit wurde eine m\"oglichkeit gefunden sogar ohne login einfach via curl die executable runterzuladen womit wir hier etwas f\"ur jeden unfassbar wertvolles gefunden haben der eben g\"unstig mit fpga anfangen will aber eben nicht seine informationen Preisgeben will. 

\subsection{SoC}
Da wir ja aber durch unserern Docker-Container dazu in der lage waren Trozdem schon zu entwickeln wurde parallel zu dem reverse Engeenerieren der website bereits die strukturellen grundladen sowie der erste code f\"ur den SoC geschrieben wo angefangen wurde sich dokumentationen die wirklich schlecht bis garnicht verf\"ugbar waren angeschaut zu dem litex framework.
\\
es wurde angefangen erstmal die ganzen tutorials durchzugehen diese zu simulieren und zu testen wo die st\"arken und die schw\"achen des frameworks liegen schnell wurde bewusst das es wirklich einfach nicht schaffbar ist in der zeit da die Dokumentationen einfach immer das gleiche waren als ob es einen bsp. code gib den einfach nur alles immer wieder versuchen zu erklaeren anstatt wirklich mal den code aufzuteilen und verschiedene schritte erkl\"aren also mussten wir uns den Source code direkt anschauen und eben aus ihm lernen und ihn verstehen und so ist daraus unserer erster prototyp Entstanden wo alles ordentlich strukturiert und gut aufgeteilt in die ordnerstruktur eingebettet worden ist dardurch das wir nun auch den code auf dem fpaga selber testen konnten konnten wir nun \"uberpr\"ufen ob der code den nun auch auf dem pysichen ger\"at funktioniert. wo wir erst zu ein paar fehlern gekommen sind zu lesen in n\"achsten kapitel ~\ref{sub:serial}.
\\ 
wir haben jedoch auch zu diesem zeitpunkt gemerkt das man so viel mehr aus diesem Projekt rausholen kann da es grade einsteigen den einstieg in SoC entwicklung mit litex immens vereinfacht durch den Docker-Container der daf\"ur sogrt das wirklich einfach alles vereinfacht wird und auch dardurch das unsere Codebasis viel besser aufgeteilt ist und nicht alles in eine datei geschoben worden ist. zudem haben wir die m\"oglichkeit gesehen daraus wirklich ein Produkt zu machen was man nutzen kann anstelle eines closed source microkontroller.

\subsubsection{das Projekt muTau}
Ab diesem moment haben wir uns gedacht wie w\"are es wenn wir ein Produkt daraus machen was eben nicht nur Einsteigen das einsteigen in SoC entwicklung vereinfacht sondern was wenn wir daraus einen microkontroller machen der opensource ist und den jeder nutzen kann und erweitern kann wie er will also haben wir uns an dieser stelle dazu entschieden das Projekt zu erweitern wir hatten ja einen Funktionierenden kern aber was will man mit dem kern wenn man nicht weis wie man ihn benutzen kann also wurde sich \"uberlegt wie man das Projekt nicht nur anschaulich sondern auch nutzbar machen k\"onnte. da kam die idee dazu eine PlatformIO intigration zu schreiben die es erm\"oglicht jedem nutzer von microkontrollern den einstieg zu erleichtern da PlatformIO eine einfache umgebung ist die bereits weit verbreitet ist und von vielen leuten bereits genutzt wird. f\"ur leute mit mehr anspruch haben die tiefer im thema drinnen sind haben wir uns \"uberlegt wie es mit einem RTOS w\"are also haben wir zwei neue Projekte zus\"atzlich angefagen eben ein RTOS Project wo wir eine Zephyr config erstellt haben und ein PlatformIO projekt wo es das ziel war eine PlatformIO integration zu schreiben f\"ur unseren SoC. weitere dinge die zus\"atzlich unternommen worden sind wie marketing und website sind hier zu finden ~\ref{sub:marketing}.
